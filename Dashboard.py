...
# Converti depuis Colab / ipywidgets → Plotly Dash
...
HEADER_STYLE = {
    ...
    'padding': '18px 28px', 'fontFamily': 'Segoe UI, Arial',
...
FILTER_BAR = {
    'background': '#E3F2FD', 'padding': '10px 24px',
    'borderLeft': '4px solid #2196F3', 'borderRadius': '6px',
    'margin': '12px 16px', 'display': 'flex',
    'alignItems': 'center', 'gap': '20px', 'flexWrap': 'wrap'
}

LABEL_STYLE = {'fontWeight': 'bold', 'color': '#263238', 'fontFamily': 'Segoe UI', 'fontSize': '13px'}
...
app.layout = html.Div(style={'backgroundColor': '#F5F7FA', 'minHeight': '100vh'}, children=[
    ...
    html.Div([
        ...
        html.Span(f" | {df['order_date'].min().date()} → {df['order_date'].max().date()}",
                  style={'fontSize': '13px', 'color': '#90CAF9', 'fontWeight': 'normal'})
    ], style=HEADER_STYLE),

    # Tabs
    dcc.Tabs(id='main-tabs', value='tab-vue1', style={'fontFamily': 'Segoe UI'},
             colors={"border": "#2196F3", "primary": "#2196F3", "background": "#ECEFF1"},
             children=[

        # ─────────────────────────────────────────────────────────
        # VUE 1 : KPIs Globaux — Direction
        # ─────────────────────────────────────────────────────────
        dcc.Tab(label='📊 Vue 1 — KPIs Direction', value='tab-vue1', children=[
        ...
        # ─────────────────────────────────────────────────────────
        # VUE 2 : Prévisions 8 semaines — Direction
        # ─────────────────────────────────────────────────────────
        dcc.Tab(label='📈 Vue 2 — Prévisions 8 sem.', value='tab-vue2', children=[
        ...
        # ─────────────────────────────────────────────────────────
        # VUE 3 : Catégories — Marketing
        # ─────────────────────────────────────────────────────────
        dcc.Tab(label='📦 Vue 3 — Catégories', value='tab-vue3', children=[
        ...
        # ─────────────────────────────────────────────────────────
        # VUE 4 : Géographie — Marketing
        # ─────────────────────────────────────────────────────────
        dcc.Tab(label='🌍 Vue 4 — Géographie', value='tab-vue4', children=[
        ...
        # ─────────────────────────────────────────────────────────
        # VUE 5 : Stocks — Opérations
        # ─────────────────────────────────────────────────────────
        dcc.Tab(label='📦 Vue 5 — Stocks', value='tab-vue5', children=[
            ...
            html.Div([
                html.Span("📦 Stock simulé (unités) :", style={**LABEL_STYLE, 'marginRight': '12px'}),
...
def update_vue1(source):
    ...
    for ci, ((val, lbl, unit, fmt), col) in enumerate(zip(items1, colors1), 1):
        fig1.add_trace(go.Indicator(
            ...
            number=dict(prefix=unit if unit=='£' else '', suffix=unit if unit=='%' else '',
    ...
    items2 = [
        (k['mape'],          f"MAPE Modèle\n< {KPI_TARGETS['mape']}%",       KPI_TARGETS['mape'], '%','.2f', True),
        (k['precision'],     f"Précision\n> {KPI_TARGETS['precision']}%",     KPI_TARGETS['precision'],'%','.1f',False),
        (k['top3_share'],    f"Top 3 Catég.\n= {KPI_TARGETS['top3_share']}%", KPI_TARGETS['top3_share'],'%','.1f',False),
    ...
    fig3 = make_subplots(rows=2, cols=1,
        subplot_titles=['📈 CA Hebdomadaire — EC India','📈 CA Hebdomadaire — PS USA'],
    ...
    if len(wec2) >= 4:
        wec2 = wec2.copy(); wec2['ma4'] = wec2['revenue'].rolling(4).mean()
        fig3.add_trace(go.Scatter(x=wec2['week'], y=wec2['ma4'], mode='lines',
            name='Tendance EC', line=dict(color=PALETTE['danger'], width=2, dash='dash')), row=1, col=1)
    ...
    if len(wps2) >= 4:
        wps2 = wps2.copy(); wps2['ma4'] = wps2['revenue'].rolling(4).mean()
        fig3.add_trace(go.Scatter(x=wps2['week'], y=wps2['ma4'], mode='lines',
            name='Tendance PS', line=dict(color=PALETTE['warning'], width=2, dash='dash')), row=2, col=1)
    ...
    fig4.add_trace(go.Bar(x=monthly_total['month'].astype(str),
        y=monthly_total['mom_growth'].fillna(0), marker_color=colors_mom, opacity=0.85,
        name='MoM %', hovertemplate='<b>%{x}</b><br>MoM: %{y:.1f}%<extra></extra>'), row=1, col=1)
    fig4.add_hline(y=0, line_dash='dot', line_color='black', line_width=1, row=1, col=1)
    fig4.add_hline(y=KPI_TARGETS['ca_growth'], line_dash='dash', line_color=PALETTE['teal'],
                   line_width=1.5, annotation_text=f'+{KPI_TARGETS["ca_growth"]}%', row=1, col=1)
    ...
    summary = {
        ...
        'Valeur': [f'{k["mape"]:.2f}%', f'{k["precision"]:.1f}%', f'{k["top3_share"]:.1f}%',
                   f'£{k["panier_moyen"]:,.0f}', f'+{k["mom_growth"]:.1f}%', f'{k["margin_avg"]:.1f}%'],
    ...
    fig6 = go.Figure(go.Table(
        header=dict(values=['<b>KPI</b>','<b>Valeur</b>','<b>Cible</b>','<b>Statut</b>','<b>Profil</b>'],
                    fill_color=PALETTE['dark'], font=dict(color='white', size=12), align='left', height=35),
        cells=dict(values=[dfs[c] for c in dfs.columns],
                   fill_color=[['white']*len(dfs), cc, ['white']*len(dfs), cc, ['white']*len(dfs)],
...
def update_vue2_static(tab):
    ...
    kpi_items = [
        (mape_ec_val,'MAPE EC',f'Cible < {MAPE_TARGET}%','%','.2f',PALETTE['success'] if mape_ec_val<MAPE_TARGET else PALETTE['danger'],MAPE_TARGET,True),
        (prec_ec,    'Précision EC','Cible > 90%','%','.1f',PALETTE['success'] if prec_ec>=90 else PALETTE['danger'],90,False),
        (mape_ps_val,'MAPE PS',f'Cible < {MAPE_TARGET}%','%','.2f',PALETTE['success'] if mape_ps_val<MAPE_TARGET else PALETTE['danger'],MAPE_TARGET,True),
        (prec_ps,    'Précision PS','Cible > 90%','%','.1f',PALETTE['success'] if prec_ps>=90 else PALETTE['danger'],90,False),
    ]
    for i, (val,titre,sous,suf,fmt,col,ref,is_mape) in enumerate(kpi_items):
        ...
        fig_kpi.add_trace(go.Indicator(
            ...
            title=dict(text=f'<b>{icon} {titre}</b><br><span style="font-size:10px;color:#777">{sous}</span>', font=dict(size=13)),
    ...
    fig_kpi.update_layout(**LAYOUT_BASE, height=220,
        title=dict(text='🎯 Performance des Modèles de Prévision (XGBoost)', font=dict(size=14)))

    # Forecast chart
    fig_fc = make_subplots(rows=2, cols=1,
        subplot_titles=['📦 EC — Historique + Prévisions 8 semaines','🛒 PS — Historique + Prévisions 8 semaines'],
    ...
    for weekly, forecast, pred_df, c_hist, c_fore, label, ridx in [
        ...
        hist = weekly.tail(52)
        fig_fc.add_trace(go.Scatter(x=hist['week'], y=hist['revenue'], mode='lines',
        ...
        if {DATE_COL,ACTUAL_COL,PRED_COL}.issubset(pred_df.columns):
            ...
            fig_fc.add_trace(go.Scatter(x=fd[DATE_COL], y=fd[PRED_COL], mode='lines',
                name=f'Fitted {label}', line=dict(color=c_fore, width=1.5, dash='dot')), row=ridx, col=1)
        ...
        fig_fc.add_trace(go.Scatter(x=forecast['week'], y=forecast['forecast'],
        ...
        fig_fc.add_shape(type='line', x0=last_iso, x1=last_iso, y0=0, y1=1, yref='paper',
                         line=dict(dash='dash', color='gray', width=1.5), row=ridx, col=1)
    ...
    def make_fc_table(fdf, label, mv):
        ...
        fdf['Tendance']      = fdf['forecast'].pct_change().mul(100).apply(
            lambda x: '📈 Hausse' if pd.notna(x) and x > 2 else ('📉 Baisse' if pd.notna(x) and x < -2 else '➡️ Stable'))
        n = len(fdf)
        cc = '#E8F5E9' if mv < MAPE_TARGET else '#FFEBEE'
        fig = go.Figure(go.Table(
            header=dict(values=['<b>#</b>','<b>Semaine</b>','<b>Prévision (£)</b>',
                                '<b>IC Bas</b>','<b>IC Haut</b>','<b>WoW %</b>','<b>Tendance</b>'],
                        fill_color=PALETTE['dark'], font=dict(color='white',size=11), align='center', height=32),
            cells=dict(values=[list(range(1,n+1)), fdf['Semaine'], fdf['Prévision (£)'],
                               fdf['IC Bas'], fdf['IC Haut'], fdf['WoW %'], fdf['Tendance']],
                       fill_color=[['white']*n,['#F5F7FA']*n,[cc]*n,['white']*n,['white']*n,
                           ['#E8F5E9' if '+' in str(v) else '#FFEBEE' if '-' in str(v) else 'white' for v in fdf['WoW %']],
                           ['white']*n],
                       font=dict(size=11), align=['center','left','right','right','right','center','center'], height=28)
        ))
        fig.update_layout(**LAYOUT_BASE, height=310,
            title=dict(text=f'📋 {label} — Prévisions | Total 8 sem: £{fdf["forecast"].sum():,.0f} | MAPE: {mv:.2f}%',
                       font=dict(size=13)))
        return fig
    ...
    if {ACTUAL_COL,PRED_COL}.issubset(pred_ec.columns):
        av = pred_ec[ACTUAL_COL].dropna(); pv = pred_ec[PRED_COL].dropna()
        n  = min(len(av), len(pv))
        fig_diag.add_trace(go.Scatter(x=av.iloc[:n], y=pv.iloc[:n], mode='markers',
            marker=dict(color=PALETTE['primary'], size=5, opacity=0.5)), row=1, col=1)
        mv = max(av.iloc[:n].max(), pv.iloc[:n].max())
        fig_diag.add_trace(go.Scatter(x=[0,mv], y=[0,mv], mode='lines',
            line=dict(color='red', dash='dash', width=1.5)), row=1, col=1)
        fig_diag.add_trace(go.Histogram(x=av.iloc[:n].values-pv.iloc[:n].values,
            nbinsx=40, marker_color=PALETTE['primary'], opacity=0.7, name='Résidus EC'), row=1, col=2)
    if {ACTUAL_COL,PRED_COL}.issubset(pred_ps.columns):
        ap = pred_ps[ACTUAL_COL].dropna(); pp = pred_ps[PRED_COL].dropna()
        np_ = min(len(ap), len(pp))
        fig_diag.add_trace(go.Histogram(x=ap.iloc[:np_].values-pp.iloc[:np_].values,
            nbinsx=40, marker_color=PALETTE['success'], opacity=0.5, name='Résidus PS'), row=1, col=2)
    ...
    fig_diag.update_layout(**LAYOUT_BASE, height=380, barmode='overlay',
        title=dict(text='🔍 Diagnostic Modèle : Réel vs Prévu & Résidus', font=dict(size=14)))

    return fig_kpi, fig_fc, fig_tec, fig_tps, fig_diag

@callback(Output('v2-sim','figure'),
          Input('v2-horizon','value'), Input('v2-ci','value'), Input('v2-src','value'))
def update_v2_sim(n_weeks, ci_pct_val, src):
    ...
    for wkly, fc_base, ch, cf, lbl in pairs:
        hist26 = wkly.tail(26)
        fig.add_trace(go.Scatter(x=hist26['week'], y=hist26['revenue'], mode='lines',
        ...
        fig.add_trace(go.Scatter(x=fc['week'], y=fc['forecast'], mode='lines+markers',
        ...
        fig.add_shape(type='line', x0=last_iso, x1=last_iso, y0=0, y1=1, yref='paper',
                      line=dict(dash='dash', color='gray', width=1.5))
    fig.update_layout(**LAYOUT_BASE, height=420, hovermode='x unified',
        title=dict(text=f'📈 Simulation — {n_weeks} semaines (IC ±{ci_pct_val:.0f}%)', font=dict(size=14)),
...
def update_vue3_static(tab):
    ...
    for ci, (val,lbl,unit,fmt,col) in enumerate(items,1):
        ...
        fig_kpi.add_trace(go.Indicator(mode='number', value=val,
            title=dict(text=f'<b>{icon} {lbl}</b>', font=dict(size=11)),
            number=dict(suffix=unit, valueformat=fmt, font=dict(size=28, color=col))), row=1, col=ci)
    ...
    for ci, (cdf, colors) in enumerate([(cat_ec, px.colors.qualitative.Set2),
                                         (cat_ps, px.colors.qualitative.Pastel)], 1):
        fig_pie.add_trace(go.Pie(labels=cdf['category'], values=cdf['revenue'], hole=0.40,
            marker=dict(colors=colors), textinfo='label+percent', textposition='outside',
            pull=[0.05 if i<3 else 0 for i in range(len(cdf))]), row=1, col=ci)
    ...
    for ci, (cdf, c_base) in enumerate([(cat_ec,PALETTE['primary']),(cat_ps,PALETTE['success'])],1):
        size_norm = (cdf['orders']/cdf['orders'].max()*40+10).tolist()
        colors_b  = [PALETTE['success'] if m>=20 else PALETTE['warning'] if m>=10 else PALETTE['danger']
                     for m in cdf['margin_pct']]
        fig_mat.add_trace(go.Scatter(x=cdf['revenue'], y=cdf['margin_pct'], mode='markers+text',
            text=cdf['category'], textposition='top center', textfont=dict(size=9),
            marker=dict(size=size_norm, color=colors_b, opacity=0.75, line=dict(color='white',width=1.5)),
            hovertemplate='<b>%{text}</b><br>CA: £%{x:,.0f}<br>Marge: %{y:.1f}%<extra></extra>'), row=1, col=ci)
        fig_mat.add_hline(y=20, line_dash='dot', line_color='gray', line_width=1, row=1, col=ci)
        fig_mat.add_vline(x=cdf['revenue'].median(), line_dash='dot', line_color='gray', line_width=1, row=1, col=ci)
    ...
    for ci, cdf in enumerate([cat_ec,cat_ps],1):
        ...
        fig_gr.add_trace(go.Bar(y=cs['category'], x=cs['mom_growth'], orientation='h',
            marker_color=[PALETTE['success'] if v>=0 else PALETTE['danger'] for v in cs['mom_growth']],
            opacity=0.85, text=[f'{v:+.1f}%' for v in cs['mom_growth']], textposition='outside'), row=1, col=ci)
        fig_gr.add_vline(x=0, line_color='black', line_width=1, row=1, col=ci)
        fig_gr.add_vline(x=10, line_dash='dash', line_color=PALETTE['teal'], line_width=1, row=1, col=ci)
    ...
    for ci, (src_df, label) in enumerate([(ec,'EC'),(ps,'PS')],1):
        ...
        fig_sea.add_trace(go.Heatmap(z=pn.values, x=pn.columns.tolist(), y=pn.index.tolist(),
    ...
    fig_disc = make_subplots(rows=1,cols=2,
        subplot_titles=['💰 Revenue Moyen avec/sans Discount','📉 Marge avec/sans Discount'])
    for ci, metric in enumerate(['revenue_mean','margin'],1):
        for dv, color, name in [(0,PALETTE['primary'],'Sans'),(1,PALETTE['warning'],'Avec')]:
            sub = disc[disc['has_discount']==dv]
            fig_disc.add_trace(go.Bar(x=sub['category'], y=sub[metric], name=name,
                marker_color=color, opacity=0.85, showlegend=(ci==1)), row=1, col=ci)
    ...
    def make_cat_table(cdf, source_label):
        ...
        for r in cdf['revenue']:
            cumul += r/total*100; cumul_pct.append(f'{cumul:.1f}%')
        rc = ['#E8F5E9' if i<3 else '#FFFFFF' for i in range(len(cdf))]
        fig = go.Figure(go.Table(
            header=dict(values=['<b>Rang</b>','<b>Catégorie</b>','<b>CA (£)</b>','<b>Part %</b>',
                                '<b>Part Cum.</b>','<b>Profit</b>','<b>Marge %</b>',
                                '<b>Commandes</b>','<b>Panier Moy.</b>','<b>MoM %</b>'],
                        fill_color=PALETTE['dark'], font=dict(color='white',size=11), align='center', height=32),
            cells=dict(values=[
                [f'#{i+1}' for i in range(len(cdf))], cdf['category'],
                [f'£{v:,.0f}' for v in cdf['revenue']], [f'{v:.1f}%' for v in cdf['ca_share']],
                cumul_pct, [f'£{v:,.0f}' for v in cdf['profit']], [f'{v:.1f}%' for v in cdf['margin_pct']],
                [f'{v:,}' for v in cdf['orders']], [f'£{v:,.0f}' for v in cdf['panier_moyen']],
                [f'{v:+.1f}%' for v in cdf['mom_growth']],
            ],
            fill_color=[rc,rc,rc,rc,rc,rc,
                [PALETTE['success'] if v>=20 else PALETTE['warning'] if v>=10 else '#FFEBEE' for v in cdf['margin_pct']],
                rc,rc,['#E8F5E9' if v>=0 else '#FFEBEE' for v in cdf['mom_growth']]],
            font=dict(size=11), height=28)
        ))
        top3 = cdf.head(3)['ca_share'].sum()
        fig.update_layout(**LAYOUT_BASE, height=max(280,len(cdf)*32+80),
            title=dict(text=f'📋 {source_label} — Top 3 = {top3:.1f}% du CA', font=dict(size=13)))
        return fig

    return fig_kpi, fig_pie, fig_mat, fig_gr, fig_sea, fig_disc, \
           make_cat_table(cat_ec,'EC India'), make_cat_table(cat_ps,'PS USA')

@callback(Output('v3-cat-dd','options'), Output('v3-cat-dd','value'), Input('v3-src-dd','value'))
...
@callback(Output('v3-drilldown','figure'), Input('v3-src-dd','value'), Input('v3-cat-dd','value'))
def update_v3_drill(src, cat_sel):
    ...
    if len(sub)==0:
        return go.Figure()
    wcat = sub.groupby('week').agg(revenue=('revenue','sum'), orders=('order_id','nunique')).reset_index()
    subcat_rev = sub.groupby('sub_category')['revenue'].sum().sort_values(ascending=False).head(8)
    fig = make_subplots(rows=1,cols=2,
        subplot_titles=[f'📈 CA Hebdo — {cat_sel}', f'📦 Top Sous-Catégories'])
    fig.add_trace(go.Scatter(x=wcat['week'], y=wcat['revenue'], mode='lines+markers',
        line=dict(color=color, width=2), fill='tozeroy', fillcolor='rgba(33,150,243,0.08)'), row=1, col=1)
    if len(wcat)>=4:
        wcat = wcat.copy(); wcat['ma4'] = wcat['revenue'].rolling(4).mean()
        fig.add_trace(go.Scatter(x=wcat['week'], y=wcat['ma4'], mode='lines',
            line=dict(color=PALETTE['danger'], width=1.5, dash='dash')), row=1, col=1)
    fig.add_trace(go.Bar(y=subcat_rev.index, x=subcat_rev.values, orientation='h',
        marker_color=color, opacity=0.8), row=1, col=2)
    fig.update_layout(**LAYOUT_BASE, height=360, showlegend=False,
        title=dict(text=f'🔍 Drill-down : {cat_sel} ({src}) | CA: £{sub["revenue"].sum():,.0f} | Marge: {sub["margin_pct"].mean():.1f}%',
                   font=dict(size=13)))
    return fig
...
def update_vue4_static(tab):
    ...
    top_reg_ec = reg_ec.iloc[0]; top_reg_ps = reg_ps.iloc[0]
    ...
    fig_reg = make_subplots(rows=1,cols=2,
        subplot_titles=['🇬🇧 EC — CA & Marge par Région','🇺🇸 PS — CA & Marge par Région'],
    ...
    for ci, (rdf,cb,cl,lbl) in enumerate([
        ...
        fig_reg.add_trace(go.Bar(y=rs['region'], x=rs['revenue'], orientation='h',
            name=f'CA {lbl}', marker_color=cb, opacity=0.85,
            text=[f'£{v:,.0f}' for v in rs['revenue']], textposition='outside'), row=1, col=ci, secondary_y=False)
        fig_reg.add_trace(go.Scatter(y=rs['region'], x=rs['margin_pct'], mode='markers+lines',
            name=f'Marge {lbl}', marker=dict(color=cl,size=9,symbol='diamond'),
            line=dict(color=cl,width=1.5,dash='dot')), row=1, col=ci, secondary_y=True)
    ...
    fig_map = go.Figure(go.Choropleth(
        ...
        hovertemplate='<b>%{text}</b><br>CA: £%{customdata[0]:,.0f}<br>Marge: %{customdata[1]:.1f}%<br>Commandes: %{customdata[2]:,}<extra></extra>',
    ...
    state_monthly = ps.groupby(['state','month'])['revenue'].sum().reset_index().sort_values('month')
    sg = {}
    ...
    fig_states.add_trace(go.Bar(y=top10['state'][::-1], x=top10['revenue'][::-1], orientation='h',
        marker_color=[PALETTE['primary'] if i<3 else PALETTE['teal'] for i in range(len(top10))][::-1],
        opacity=0.85, text=[f'£{v:,.0f}' for v in top10['revenue'][::-1]], textposition='outside'), row=1, col=1)
    fig_states.add_trace(go.Bar(y=top10['state'][::-1], x=top10['mom_growth'][::-1], orientation='h',
        marker_color=[PALETTE['success'] if v>=0 else PALETTE['danger'] for v in top10['mom_growth'][::-1]],
        opacity=0.85, text=[f'{v:+.1f}%' for v in top10['mom_growth'][::-1]], textposition='outside'), row=1, col=2)
    ...
    for ci, (cities, color) in enumerate([(top_cities_ec,PALETTE['primary']),(top_cities_ps,PALETTE['success'])],1):
        fig_cities.add_trace(go.Bar(y=cities['city'][::-1], x=cities['revenue'][::-1], orientation='h',
            marker_color=color, opacity=0.85,
            text=[f'£{v:,.0f}' for v in cities['revenue'][::-1]], textposition='outside'), row=1, col=ci)
    ...
    fig_time = make_subplots(rows=1,cols=2,
        subplot_titles=['📅 EC — CA Mensuel par Région','📅 PS — CA Mensuel par Région'])
    ...
    for ci, (src_df,colors,lbl) in enumerate([(ec,ec_cr,'EC'),(ps,ps_cr,'PS')],1):
        ...
        for i, reg in enumerate(sorted(src_df['region'].unique())):
            ...
            fig_time.add_trace(go.Scatter(x=sub['month'], y=sub['revenue'], mode='lines',
                name=f'{reg} ({lbl})', line=dict(color=colors[i%len(colors)], width=2)), row=1, col=ci)
    ...
    fig_hm = make_subplots(rows=1,cols=2,
        subplot_titles=['🌡️  EC — Région × Catégorie','🌡️  PS — Région × Catégorie'])
    for ci, (src_df,lbl) in enumerate([(ec,'EC'),(ps,'PS')],1):
        pivot = src_df.groupby(['region','category'])['revenue'].sum().unstack(fill_value=0)
        pn    = pivot.div(pivot.max(axis=1), axis=0) * 100
        fig_hm.add_trace(go.Heatmap(z=pn.values, x=pn.columns.tolist(), y=pn.index.tolist(),
            colorscale='Blues', zmin=0, zmax=100,
            text=[[f'£{v:,.0f}' for v in row] for row in pivot.values],
            texttemplate='%{text}', textfont=dict(size=8), showscale=(ci==2)), row=1, col=ci)
    ...
    def make_reg_table(rdf, sl):
        ...
        for r in rdf['revenue']:
            cumul += r/total*100; cl.append(f'{cumul:.1f}%')
        rc = ['#E8F5E9' if i==0 else '#FFFFFF' for i in range(len(rdf))]
        fig = go.Figure(go.Table(
            header=dict(values=['<b>Rang</b>','<b>Région</b>','<b>CA (£)</b>','<b>Part %</b>',
                                '<b>Part Cum.</b>','<b>Profit</b>','<b>Marge %</b>',
                                '<b>Commandes</b>','<b>Clients</b>','<b>MoM %</b>'],
                        fill_color=PALETTE['dark'], font=dict(color='white',size=11), align='center', height=32),
            cells=dict(values=[
                [f'#{i+1}' for i in range(len(rdf))], rdf['region'],
                [f'£{v:,.0f}' for v in rdf['revenue']], [f'{v:.1f}%' for v in rdf['ca_share']], cl,
                [f'£{v:,.0f}' for v in rdf['profit']], [f'{v:.1f}%' for v in rdf['margin_pct']],
                [f'{v:,}' for v in rdf['orders']], [f'{v:,}' for v in rdf['clients']],
                [f'{v:+.1f}%' for v in rdf['mom_growth']],
            ],
            fill_color=[rc,rc,rc,rc,rc,rc,
                [PALETTE['success'] if v>=20 else PALETTE['warning'] if v>=10 else '#FFEBEE' for v in rdf['margin_pct']],
                rc,rc,['#E8F5E9' if v>=0 else '#FFEBEE' for v in rdf['mom_growth']]],
            font=dict(size=11), height=28)
        ))
    ...
    return (fig_kpi, fig_reg, fig_map, fig_states, fig_cities,
            fig_time, fig_hm, make_reg_table(reg_ec,'EC India'), make_reg_table(reg_ps,'PS USA'))

@callback(Output('v4-region-dd','options'), Output('v4-region-dd','value'), Input('v4-src-geo','value'))
...
@callback(Output('v4-drilldown','figure'),
          Input('v4-src-geo','value'), Input('v4-region-dd','value'), Input('v4-metric','value'))
def update_v4_drill(src, reg, metric):
    src_df = ec if src=='EC' else ps
    color  = PALETTE['primary'] if src=='EC' else PALETTE['success']
    sub    = src_df[src_df['region']==reg].copy()
    if len(sub)==0: return go.Figure()
    ms = sub.groupby('month').agg(revenue=('revenue','sum'), profit=('profit','sum'),
    ...
    fig.add_trace(go.Scatter(x=ms['month'], y=y_vals, mode='lines+markers',
        line=dict(color=color, width=2.5), marker=dict(size=7),
        fill='tozeroy', fillcolor='rgba(33,150,243,0.08)'), row=1, col=1)
    fig.add_trace(go.Bar(y=cat_sub.index, x=cat_sub.values, orientation='h',
        marker_color=color, opacity=0.8), row=1, col=2)
    fig.add_trace(go.Bar(y=city_sub.index, x=city_sub.values, orientation='h',
        marker_color=PALETTE['teal'], opacity=0.8), row=1, col=3)
    fig.update_layout(**LAYOUT_BASE, height=360, showlegend=False,
        title=dict(text=f'🔍 Drill-down : {reg} ({src}) | CA: £{sub["revenue"].sum():,.0f} | Marge: {sub["margin_pct"].mean():.1f}%',
                   font=dict(size=13)))
    return fig
...
def update_vue5_static(tab):
    ...
    fig_kpi = make_subplots(rows=1,cols=4,specs=[[{'type':'indicator'}]*4])
    items = [
        (kpi_stock_ec['couv_moy'], f'Couverture EC\n≥ {STOCK_COVERAGE_TARGET} sem.',' sem','.1f',
         PALETTE['success'] if kpi_stock_ec['couv_moy']>=STOCK_COVERAGE_TARGET else PALETTE['danger']),
        (kpi_stock_ps['couv_moy'], f'Couverture PS\n≥ {STOCK_COVERAGE_TARGET} sem.',' sem','.1f',
         PALETTE['success'] if kpi_stock_ps['couv_moy']>=STOCK_COVERAGE_TARGET else PALETTE['danger']),
        (kpi_stock_ec['n_alertes'], f'Alertes EC\n({kpi_stock_ec["n_total"]} catégs.)','','d',
         PALETTE['success'] if kpi_stock_ec['n_alertes']==0 else PALETTE['warning'] if kpi_stock_ec['n_alertes']<=2 else PALETTE['danger']),
        (kpi_stock_ps['n_alertes'], f'Alertes PS\n({kpi_stock_ps["n_total"]} catégs.)','','d',
         PALETTE['success'] if kpi_stock_ps['n_alertes']==0 else PALETTE['warning'] if kpi_stock_ps['n_alertes']<=2 else PALETTE['danger']),
    ]
    for ci, (val,lbl,unit,fmt,col) in enumerate(items,1):
        ...
        fig_kpi.add_trace(go.Indicator(mode='number', value=val,
            ...
            number=dict(suffix=unit, valueformat=fmt, font=dict(size=28, color=col))), row=1, col=ci)
    fig_kpi.update_layout(**LAYOUT_BASE, height=160,
        title=dict(text='🎯 KPIs Opérationnels — Couverture & Alertes', font=dict(size=14)))

    # Coverage
    status_colors = {'🔴 RUPTURE IMMINENTE':PALETTE['danger'],'🟡 STOCK FAIBLE':PALETTE['warning'],
                     '🟢 STOCK OK':PALETTE['success'],'🔵 SURSTOCKAGE':PALETTE['primary']}
    fig_cov = make_subplots(rows=1,cols=2,
        subplot_titles=['📦 EC — Taux de Couverture (semaines)','📦 PS — Taux de Couverture (semaines)'])
    for ci, sdf in enumerate([stock_ec,stock_ps],1):
        ds = sdf.sort_values('taux_couverture', ascending=True)
        fig_cov.add_trace(go.Bar(y=ds['category'], x=ds['taux_couverture'], orientation='h',
            marker_color=[status_colors.get(s,PALETTE['teal']) for s in ds['statut_stock']],
            opacity=0.85, text=[f'{v:.1f} sem' for v in ds['taux_couverture']],
            textposition='outside'), row=1, col=ci)
        fig_cov.add_vline(x=STOCK_COVERAGE_TARGET, line_dash='dash', line_color='black', line_width=2, row=1, col=ci)
    ...
    def make_alert_tbl(sdf, sl):
        ...
        sdf['Priorité'] = sdf['taux_couverture'].apply(
            lambda x: '🔴 URGENT' if x<1 else ('🟡 ÉLEVÉE' if x<STOCK_COVERAGE_TARGET else '🟢 Normal'))
        rfill = ['#FFEBEE' if '🔴' in s else '#FFF8E1' if '🟡' in s else '#E3F2FD' if '🔵' in s else '#F1F8E9'
                 for s in sdf['statut_stock']]
        fig = go.Figure(go.Table(
            header=dict(values=['<b>Priorité</b>','<b>Catégorie</b>','<b>Stock Actuel</b>',
                                '<b>Stock Recommandé</b>','<b>Stock Sécu.</b>','<b>Pt Réappro</b>',
                                '<b>Couverture</b>','<b>Rotation</b>','<b>Statut</b>','<b>Alerte</b>'],
                        fill_color=PALETTE['dark'], font=dict(color='white',size=11), align='center', height=32),
            cells=dict(values=[
                ...
                sdf['statut_stock'],
                ['🚨 COMMANDER' if a else '✅ OK' for a in sdf['alerte_reappro']],
            ],
            fill_color=[rfill]*9 + [['#FFEBEE' if a else '#F1F8E9' for a in sdf['alerte_reappro']]],
            font=dict(size=11), height=28)
        ))
        ...
        fig.update_layout(**LAYOUT_BASE, height=max(280,len(sdf)*32+80),
            title=dict(text=f'📋 {sl} | 🔴 Urgent: {n_urg} | 🟡 Élevé: {n_elv}', font=dict(size=13)))
        return fig

    # Demand vs stock
    fig_dem = make_subplots(rows=1,cols=2,
        subplot_titles=['📈 EC — Demande 8 sem. vs Stock','📈 PS — Demande 8 sem. vs Stock'])
    for ci, (fdf,sdf,color) in enumerate([(demand_fc_ec,stock_ec,PALETTE['primary']),
                                           (demand_fc_ps,stock_ps,PALETTE['success'])],1):
        td = fdf.groupby('category')['forecast'].sum().reset_index()
        td.columns = ['category','demand_8w']
        merged = td.merge(sdf[['category','stock_actuel','stock_recommande']], on='category', how='left').sort_values('demand_8w', ascending=True)
        fig_dem.add_trace(go.Bar(y=merged['category'], x=merged['demand_8w'], orientation='h',
            name='Demande 8 sem', marker_color=color, opacity=0.7), row=1, col=ci)
        fig_dem.add_trace(go.Scatter(y=merged['category'], x=merged['stock_actuel'], mode='markers',
            name='Stock Actuel',
            marker=dict(color=[PALETTE['success'] if s>=d else PALETTE['danger']
                               for s,d in zip(merged['stock_actuel'],merged['demand_8w'])],
                        size=12, symbol='diamond', line=dict(color='white',width=1.5))), row=1, col=ci)
    fig_dem.update_layout(**LAYOUT_BASE, height=440, barmode='overlay',
        title=dict(text='📊 Demande Prévue 8 sem vs Stock Disponible', font=dict(size=14)),
        legend=dict(orientation='h', y=-0.12))
    ...
    fig_rot.add_trace(go.Bar(y=rot_mean.index, x=rot_mean.values, orientation='h',
        marker_color=[PALETTE['success'] if v>=4 else PALETTE['warning'] if v>=2 else PALETTE['danger']
                      for v in rot_mean.values],
        opacity=0.85, text=[f'{v:.1f}x' for v in rot_mean.values], textposition='outside'), row=1, col=1)
    fig_rot.add_vline(x=4, line_dash='dash', line_color=PALETTE['teal'], line_width=1.5, row=1, col=1)
    cv_mean = all_stock.groupby('category')['cv'].mean().sort_values(ascending=False)
    fig_rot.add_trace(go.Bar(y=cv_mean.index, x=cv_mean.values, orientation='h',
        marker_color=[PALETTE['danger'] if v>50 else PALETTE['warning'] if v>30 else PALETTE['success']
                      for v in cv_mean.values],
        opacity=0.85, text=[f'{v:.0f}%' for v in cv_mean.values], textposition='outside'), row=1, col=2)
    fig_rot.add_vline(x=30, line_dash='dash', line_color='gray', line_width=1.5, row=1, col=2)
    ...
    fig_evo = make_subplots(rows=1,cols=2,
        subplot_titles=['📅 EC — Demande Hebdo par Catégorie','📅 PS — Demande Hebdo par Catégorie'])
    for ci, (src_df,colors,lbl) in enumerate([
        ...
        for i, cat in enumerate(sorted(src_df['category'].unique())):
            ...
            fig_evo.add_trace(go.Scatter(x=sub['week'], y=sub['quantity'], mode='lines',
                name=f'{cat} ({lbl})', line=dict(color=colors[i%len(colors)], width=1.5), opacity=0.8), row=1, col=ci)
    fig_evo.update_layout(**LAYOUT_BASE, height=400, hovermode='x unified',
        title=dict(text='📈 Évolution Demande Hebdomadaire par Catégorie', font=dict(size=14)),
        legend=dict(orientation='h', y=-0.15, font=dict(size=9)))

    return (fig_kpi, fig_cov, make_alert_tbl(stock_ec,'EC India'),
            make_alert_tbl(stock_ps,'PS USA'), fig_dem, fig_rot, fig_evo)

@callback(Output('v5-cat-dd','options'), Output('v5-cat-dd','value'), Input('v5-src-stock','value'))
def update_v5_cat_dd(src):
    opts = sorted(ec['category'].unique()) if src=='EC' else sorted(ps['category'].unique())
    return [{'label':c,'value':c} for c in opts], opts[0]

@callback(Output('v5-gauge','figure'),
          Input('v5-src-stock','value'), Input('v5-cat-dd','value'), Input('v5-stock-slider','value'))
def update_v5_gauge(src, cat, stock):
    ...
    fig = go.Figure(go.Indicator(
        ...
        title=dict(text=f'<b>Couverture — {cat} ({src})</b><br><span style="font-size:12px">{statut}</span>',
                   font=dict(size=13)),
        number=dict(suffix=' semaines', valueformat='.1f', font=dict(size=30, color=s_color)),
        delta=dict(reference=STOCK_COVERAGE_TARGET, valueformat='.1f',
                   increasing=dict(color=PALETTE['success']), decreasing=dict(color=PALETTE['danger'])),
        gauge=dict(
            axis=dict(range=[0,10]),
            bar=dict(color=s_color, thickness=0.25),
            bgcolor='#F0F0F0',
            steps=[dict(range=[0,1],color='#FFEBEE'), dict(range=[1,2],color='#FFF8E1'),
                   dict(range=[2,6],color='#E8F5E9'), dict(range=[6,10],color='#E3F2FD')],
            threshold=dict(line=dict(color='black',width=2), thickness=0.75, value=STOCK_COVERAGE_TARGET)
        )
    ))
    fig.update_layout(**LAYOUT_BASE, height=320,
        title=dict(text=f'📦 Simulateur Stock — Stock: {stock:,} u | Demande moy: {demand_mean:.0f} u/sem',
                   font=dict(size=13)))
    return fig
